...
Rath::Pipeline::~Pipeline() {
	...
	vkDestroyPipeline(device.getDevice(), particlePipeline, nullptr);
	vkDestroyPipelineLayout(device.getDevice(), particlePipelineLayout, nullptr);
...
void Rath::Pipeline::createParticlePipeline() {
	...
	auto bindingDescriptions = Rath::Particle::getBindingDescription();
	auto attributeDescriptions = Rath::Particle::getAttributeDescriptions();
	...
	if (vkCreateGraphicsPipelines(device.getDevice(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &particlePipeline) != VK_SUCCESS) {
